...
const createPet = (req: Request, res: Response) => {
  let pet = req.body;
  db.query(
    "INSERT INTO pets (name, category_id, category_name, photoUrls, tags_id, tags_name, status) VALUES ( ? , ? , ? , ? , ? , ? , ? )",
    [
      pet.name,
      pet.category_id,
      pet.category_name,
...
const deletePet = (req: Request, res: Response) => {
  let pet_id = req.params.id;
  db.query("DELETE FROM pets WHERE id= ?", pet_id, (error, result) => {
